...
from models.layers import BertEncoder
...
class QAModel(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.qa_cross_att = BertEncoder(num_layers=1)
    ...
    Returns:
        ...
        init_output = outputs[0]

        input_len = input_ids.size(1)
        padding_mask = (query_mask + ans_mask).eq(0)
        padding_mask = padding_mask.unsqueeze(1).expand(-1, input_len, -1)
        sequence_output, attn = self.qa_cross_att(init_output, padding_mask)
        ...
        outputs = (start_logits, end_logits, attn)
